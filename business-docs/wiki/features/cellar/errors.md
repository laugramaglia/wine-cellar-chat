...
| --- | --- | --- | --- |
| Missing or malformed `Authorization` header | `401` at the Worker edge | Connection refused; no tool list at all (`README.md:145`) | Supply a valid token |
| Unknown, revoked, or expired token | `401` | Same | New token from an admin — [[token-administration-index]] |
| Token's user is `status != active` | `401` | Same (`README.md:331`) | Admin reactivates — [[user-administration-index]] |
| Caller lacks `cellar:read` / `cellar:write` (e.g. a `guest`) | MCP error | `Permission denied: '<tool>' requires '<permission>'; your role is '<role>'.` (`README.md:139`) | None from the client; the message is deliberately explicit so the agent reports it instead of retrying (`README.md:140`) |
| Token scoped narrower than the user's role, e.g. `catalog:read` only | MCP error, same shape | Same. Tested in the definition of done: a `catalog:read` token is refused by `cellar_add` even though its user is a `member` (`README.md:412`) | Use a token with the scope |
...
There is no "you do not own this cellar" error for reads, because there is no way to ask for someone else's: no non-admin tool takes a `user_id`, and handlers read the caller from `props` (`README.md:155`, `README.md:336`). The class of error is designed out rather than caught. See [[cellar-validations]] and [[security]].
...
None specified, in either direction. The permission-denial message is worded to stop an agent retrying in a loop (`README.md:140`) — that is the only retry-shaped rule in the project.
