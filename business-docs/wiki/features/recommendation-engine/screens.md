...
(`README.md:39`). The product is a remote MCP server on Cloudflare Workers with a single
Streamable HTTP endpoint at `/mcp` (`README.md:25`). Claude Desktop, Claude Code, Gemini,
Cursor or a bespoke agent connect to it (`README.md:6-8`); each of those renders its own
...
JSON of `README.md:257-271` turned into prose by a model we do not control.

| Surface | What it is | Who controls it |
| --- | --- | --- |
| The `wine_recommend` result object | Ranked entries with `wine`, `score`, `in_cellar`, `quantity`, `reasons`, `penalties` (`README.md:257-271`) | This feature — see [[recommendation-engine-api]] |
...
(`README.md:422-423`) would be the first thing resembling a composed surface; neither
exists.
