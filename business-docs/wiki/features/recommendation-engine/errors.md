...
| --- | --- | --- | --- |
| Missing or malformed `Authorization` header | `401` at the Worker edge (`README.md:329`) | Connection refused; no tool list at all (`README.md:145`) | Supply a valid token. [[authentication-index]] |
| Unknown, revoked, or expired token | `401` (`README.md:330`) | as above | Have an admin issue a new key ([[token-administration-index]]) |
| Token's user is `status = suspended` | `401` (`README.md:331`, `README.md:144`) | as above | Admin reactivates the account |
| Caller lacks `recommend` | MCP error, `Permission denied: 'wine_recommend' requires 'recommend'; your role is '<role>'.` (pattern from `README.md:139-140`) | An explicit refusal the agent reports rather than retrying | Cannot occur through role alone — all three roles hold `recommend` (`README.md:126`). Only a narrowed token `scopes` list can cause it (`README.md:113-114`). |
...
| Invalid input value (e.g. `wine_type: "purple"`) | Zod schema rejection at the tool boundary (`README.md:363`) | A schema error from the MCP layer | Correct the argument. See [[recommendation-engine-validations]]. |
...
| `last_used_at` update | Any failure, by design — best-effort via `ctx.waitUntil` (`README.md:341`) | Nothing. Belongs to [[authentication-index]]. |
...
them instead of retrying (`README.md:139-141`).
...
| No stated behaviour when the Neon HTTP call fails mid-scoring. | Shared concern; nothing in `README.md` covers database failure for any tool. |
