...
| [ADR-0015](../../decisions/0015-closed-enumerations-are-database-types.md) | `body`, `tannin` and `acidity` share one Postgres `intensity` type whose declaration order is the scale's order | Gives the palate-fit component (0.25) an ordering it can compare in SQL — `body >= 'medium'` and `ORDER BY body` now work. It supplies the **order**, not the **distance**: the numeric step between adjacent points is still open below |
| [ADR-0021](../../decisions/0021-wine-search-is-full-text-plus-trigram.md) | Search ranking is deterministic `ts_rank` over a stored weighted `tsvector` | Extends [ADR-0004](../../decisions/0004-a-deterministic-rule-based-engine.md)'s determinism requirement to the retrieval step that feeds the engine candidates, and rules out embeddings as the matching path for the MVP |
...
Decisions this feature still needs. Each is a genuine gap in [[mvp-spec]], not a detail
...
| --- | --- | --- |
| **What is the numeric distance between adjacent points on the 5-point scale?** `low \| medium_minus \| medium \| medium_plus \| high` (`business-docs/wiki/shared/mvp-spec.md:102`) is an ordered enum with no metric. Palate fit is "distance on the 5-point scale" (`business-docs/wiki/shared/mvp-spec.md:309`). | Nobody has chosen. Linear `0, 0.25, 0.5, 0.75, 1` is the obvious default but is a decision, not a reading of the spec. It also needs a rule for `tannin = null`, which is normal for whites (`business-docs/wiki/shared/mvp-spec.md:103`) — presumably "drop the sub-component", per ADR-0006. | **0.25** |
| **What is in the built-in food→style pairing table?** Three examples and an ellipsis: red meat→tannic red, shellfish→high-acid white, spicy→off-dry aromatic (`business-docs/wiki/shared/mvp-spec.md:308`). | The table is the largest input to the heaviest component and is entirely undocumented. Planned home `src/engine/pairings.ts` (`business-docs/wiki/shared/mvp-spec.md:382`). Someone must write and own the list — it is a domain judgement, not an implementation detail. | **0.30** |
...
| **How are ties broken?** | Determinism (`business-docs/wiki/shared/mvp-spec.md:317`) demands a total order. Any implicit key is an unexplained ordering influence, forbidden by ADR-0005. A stated key — say `drink_until` ascending, then `id` — would need its own reason string or an explicit exemption. | ordering |
| **What happens to a wine with zero usable components?** | Renormalization divides by a zero weight sum, and the entry would carry no reasons, breaking `business-docs/wiki/shared/mvp-spec.md:421`. ADR-0006 names this as a degenerate case it does not address. Options: exclude it, floor the score, or give it a "we know almost nothing about this bottle" reason. | edge |
| **Does `use_prefs: false` also disable the `avoid` and `dislikes` hard filters?** | Read literally it does, since both filters are defined over `prefs` (`business-docs/wiki/shared/mvp-spec.md:300-301`). `avoid` carries allergens (`business-docs/wiki/shared/mvp-spec.md:82`), so this is a safety question, not a preference one. | filters |
