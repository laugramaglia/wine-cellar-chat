...
of the whole thing (`README.md:273-274`).
...
spec gives exactly five, all inside one illustrative result (`README.md:263-270`).

| Key | Source text | Placeholders | Component |
| --- | --- | --- | --- |
| `reason.personal-history.grape` | `Malbec matches a grape you rate highly (avg 92 over 4 reviews)` | grape name, mean rating, review count | Personal history, 0.20 (`README.md:264`) |
| `reason.food-pairing.listed` | `Food pairing: lamb is listed in this wine's pairings` | requested food | Food pairing, 0.30 (`README.md:265`) |
| `reason.budget.inside-band` | `$28 is inside your $0–40 budget` | price, band min, band max | Budget fit, 0.05 (`README.md:266`) |
| `reason.drink-window.closing` | `Drink window closes in 5 months` | months remaining | Drink-window urgency, 0.05 (`README.md:267`) |
| `penalty.palate.tannin-high` | `Higher tannin than your usual preference` | — | Palate fit, 0.25 (`README.md:269`) |
...
| --- | --- | --- |
| `avg 92 over 4 reviews` | An arithmetic mean of exactly 4 of the caller's reviews is 92 | **Copy only.** No definition of the aggregate is given: mean or median, this wine or the grape across producers, the caller's reviews only or all users'. `wine_get` returns "aggregate rating across all users" (`README.md:176`) while personal history is explicitly the caller's own (`README.md:296`) — the string must not silently mix them. |
| `a grape you rate highly` | The grape is above some threshold | **Copy only.** `README.md:296` says "a grape you rate 90+ pulls hard", which reads as a threshold, but 90 is stated as an illustration of magnitude, not as a boundary. The string says "highly"; nothing defines it. |
| `lamb is listed in this wine's pairings` | `"lamb"` appears in the wine's `food_pairings` array | **Partly enforced** — this exact claim is checkable against the column (`README.md:72`). But food pairing also scores via the built-in table (`README.md:294`), and there is no example string for *that* path. A match by table must not claim the wine "lists" the food. |
| `$28 is inside your $0–40 budget` | The wine's price is within `prefs.budget_min`..`budget_max` | **Enforced by the rule** — inside the band scores `1.0` (`README.md:298`). The `$` and the `0–40` render `budget_min`/`budget_max` (`README.md:65`); the currency is never specified anywhere in the spec. |
| `Drink window closes in 5 months` | `drink_until` is ~5 months away | **Enforced by data** — `drink_until` is a real column (`README.md:77`). But this string can only be true for a cellar item; emitting it for a catalogue-only entry would be a lie, and nothing guards it besides the component being cellar-only (`README.md:299`). |
| `Higher tannin than your usual preference` | The wine's `tannin` exceeds `prefs.tannin` | **Copy only.** "Higher" implies an ordering over the 5-point scale, and the distance metric on that scale is undefined. Also note `tannin` is `null` for most whites (`README.md:89`) — a null must produce *no* string, not "higher". |

## The hard guarantee

> Every recommendation carries at least one non-empty `reasons` entry (`README.md:407`).
...
`$` hard-coded as the currency symbol (`README.md:266`). No localization system is
mentioned anywhere in `README.md`, and no `locale` argument exists on any tool.
...
| Currency is never specified. | `avg_price`, `purchase_price`, `budget_min`/`budget_max` are bare numbers (`README.md:65`, `README.md:72`, `README.md:76`) rendered with `$`. |
