...
| --- | --- | --- | --- | --- | --- |
| 1 | No `Authorization` header, or malformed | 1 (`business-docs/wiki/shared/mvp-spec.md:343`) | `401` | connection refused, no tool list, nothing (`business-docs/wiki/shared/mvp-spec.md:159`) | supply a well-formed `Authorization: Bearer <token>` |
| 2 | Token unknown — hash matches no row | 2 (`business-docs/wiki/shared/mvp-spec.md:344`) | `401` | as above | obtain a token from an admin |
| 3 | Token revoked — `revoked_at` set | 2 (`business-docs/wiki/shared/mvp-spec.md:344`) | `401` | as above | ask an admin to issue a new one; revocation is terminal |
| 4 | Token expired — `expires_at` passed | 2 (`business-docs/wiki/shared/mvp-spec.md:344`) | `401` | as above | ask an admin to issue a new one |
| 5 | User not active — `status != active` | 3 (`business-docs/wiki/shared/mvp-spec.md:345`) | `401` | as above | ask an admin to reinstate the account |

The rejection happens **before any permission check** (`business-docs/wiki/shared/mvp-spec.md:157`) and before the MCP session exists. There is no tool list, no partial capability, no degraded mode.
...
The corresponding asymmetry — [[error-codes]] states it directly — is that **tool-level** permission denials are deliberately verbose: `Permission denied: 'user_create' requires 'admin:users'; your role is 'member'.` (`business-docs/wiki/shared/mvp-spec.md:153-154`). That is safe precisely because it comes *after* this page's checks have passed. Identity is already established; naming the missing permission tells an authenticated caller nothing they could not read in the documentation, and it stops an LLM client retrying in a loop.
...
| --- | --- | --- |
| `last_used_at` write | any failure of the `ctx.waitUntil` update (`business-docs/wiki/shared/mvp-spec.md:355`) | nothing — the request succeeds. The timestamp is simply not written, and nothing records that it was not |
| The `401` itself | which of the five conditions fired | one undifferentiated failure. **Deliberate** — see above |

The first row is an **accidental** silence with a real consequence: `user_list` "last activity" (`business-docs/wiki/shared/mvp-spec.md:227`) and `token_list` "last used" (`business-docs/wiki/shared/mvp-spec.md:244`) both read this field. A stale or null `last_used_at` is not evidence that a token went unused. Anyone pruning tokens on that basis is acting on an unreliable signal. Recorded in [[divergences]].
...
| Rate limiting | **None is described anywhere** ([[mvp-spec]] has no mention). Repeated `401`s are unbounded and uncounted. |
| Logging of failures | Only *admin actions* are audited (`business-docs/wiki/shared/mvp-spec.md:359-361`). Nothing says whether a failed authentication is logged at all — so there is no specified way to notice a brute-force attempt. See [[audit-logging]]. |
| Database error during steps 2–3 | Unspecified, as above. |
| Timing differences between conditions | A naive lookup could make condition 2 measurably faster than 3–5. With 32 bytes of entropy (`business-docs/wiki/shared/mvp-spec.md:335`) this is very likely academic — the search space is not walkable regardless — but it is the one channel through which the opaque `401` could leak what it is designed to hide. Worth a constant-time comparison; not worth alarm. |
| SHA-256 collision / duplicate `token_hash` | No uniqueness constraint is declared on `api_tokens` (`business-docs/wiki/shared/mvp-spec.md:74-75`), unlike `wines` (`business-docs/wiki/shared/mvp-spec.md:88`). What a multi-row match resolves to is undefined. |
