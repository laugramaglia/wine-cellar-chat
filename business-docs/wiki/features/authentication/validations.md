...
Everything here runs **server-side, at the Worker edge**, before any tool schema is consulted. The MCP client validates nothing: it pastes a header it was given (`README.md:385`).

## Rules

| Input | Rule | Enforced where | Message on failure |
| --- | --- | --- | --- |
| `Authorization` header | must be present | step 1 (`README.md:329`) | none — bare `401` |
| `Authorization` header | must be well-formed `Bearer <token>` | step 1 (`README.md:329`) | none — bare `401` |
| token | SHA-256 hash must match an `api_tokens` row | step 2 (`README.md:330`) | none — bare `401` |
| token | `revoked_at` must be null | step 2 (`README.md:330`) | none — bare `401` |
| token | `expires_at` must be null or in the future | step 2 (`README.md:330`) | none — bare `401` |
| user | `status` must be `active` | step 3 (`README.md:331`) | none — bare `401` |
| `token.scopes` | applied as an intersection, so a scope outside the role grants nothing | step 4 (`README.md:332`) | none — silently narrowed |

Every message column is empty on purpose. See [[authentication-errors]] for why.

## What "malformed" is not defined to mean

`README.md:329` says "missing or malformed → `401`" and stops. The following are all unspecified, and each is a decision someone will make by accident while implementing:
...
| Token uniqueness in storage | No `UNIQUE (token_hash)` declared on `api_tokens` (`README.md:60-61`), though `wines` declares one (`README.md:74`). A duplicate-hash lookup is undefined. |
...
| Token age | `expires_at` is optional (`README.md:325`), and no rotation policy is stated. The default is an immortal credential. |
