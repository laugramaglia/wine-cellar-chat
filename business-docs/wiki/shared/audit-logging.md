...
updated: 2026-08-29
code_refs:
  - business-docs/wiki/shared/mvp-spec.md:357
---

# Audit logging

Source: `business-docs/wiki/shared/mvp-spec.md:359-361`.
...
**Tokens, in plaintext or hashed** (`business-docs/wiki/shared/mvp-spec.md:361`). `metadata` may name a token's `label` and `id`; it must never carry its value. See [[security]].
...
- No retention rule.
- Whether the write is on the request path or deferred via `ctx.waitUntil` — the specification only says that for `last_used_at` (`business-docs/wiki/shared/mvp-spec.md:355`).
